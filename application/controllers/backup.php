...
class Backup extends CI_Controller {
    ...
    function index() {
        ...
        $data['results'] = $this->backup->get_backup_list('backup', '*', '', $this->backup->count('backup'), $this->uri->segment(3));
        $data['count_result'] = $this->backup->count('backup');
        ...
        // Set the page title
        $this->layouts->set_title('Site Management');
        ...
        $this->layouts->viewAdmin('admin/site_mgmt_view', $data);
    }
     ...
     * optimize_db
     *
     * Function is working but please add notification to admin user
     * 
     */
...
//        }
        ...
        if ($this->input->post('optimized_db')) {
            $this->admin_model->optimized_dbase();
        }
        ...
        // Get database detail
        $data['db_size'] = $this->admin_model->getDbSize();
        $data['db_overhead'] = $this->admin_model->getDbOverhead();
        $data['db_last_optimized'] = $this->admin_model->getDbLastOptimized();
        
        
        // Set the page title
        $this->layouts->set_title('Optimized Database');
        ...
        $this->layouts->viewAdmin('admin/optimized_db', $data);
    }
